...
def sigmac_generate(sigma):
    ...
    sigmac_output = subprocess.run(["sigmac", "-t", "es-qs", "-c", "playbook/sysmon.yml",
                                    "-c", "playbook/threatcode-baseline.yml", "--backend-option", "keyword_whitelist=source.ip,destination.ip,source.port,destination.port,message,rule.uuid", "--backend-option", "keyword_field=.keyword", "--backend-option", "analyzed_sub_field_name=.security", "--backend-option", "wildcard_use_keyword=false", temp_file.name],
