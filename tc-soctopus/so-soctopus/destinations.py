...
def createHiveAlert(esid):
    ...
    for item in search['hits']['hits']:
        ...
        if event['module'] == 'ids':
            ...
            description = "`NIDS Dashboard:` \n\n <https://" + masterip + f"/kibana/tc-soctopus/kibana#/dashboard/ed6f7e20-e060-11e9-8f0c-2ddbf5ed9290?_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-24h,mode:quick,to:now))&_a=(columns:!(_source),index:'*:{es_index}',interval:auto,query:(query_string:(analyze_wildcard:!t,query:'sid:" + sid + "')),sort:!('@timestamp',desc))> \n\n `IPs: `" + src + ":" + srcport + "-->" + dst + ":" + dstport + "\n\n `Signature:`" + alert + "\n\n `PCAP:` " + "https://" + masterip + "/kibana/tc-soctopus//sensoroni/threatcode/joblookup?redirectUrl=/sensoroni/&esid=" + es_id
        ...
        # Sysmon logs
            ...
            title = f'New {event["module"]}_{event["dataset"]} Event From Threat Code'
        form = DefaultForm()
        artifact_string = jsonpickle.encode(artifacts)
...
def createRTIRIncident(esid):
    ...
    for result in search['hits']['hits']:
        ...
        description = str(message)
        event = result['event']
        rtir_subject = f'New {event["module"]}_{event["dataset"]} Event From Threat Code'
...
def createFIREvent(esid):
    ...
    for result in search['hits']['hits']:
        ...
        subject = f'New {event["module"]}_{event["dataset"]} Event From Threat Code'
